...
export class TenantAdminController {
    ...
    @Patch(':id')
    async update(@Param('id') id: string, @Body() dto: UpdateTenantAdminDto, @Request() req) {
        assertPlatformAdmin(req.user);
        return this.service.updateTenant(req.user.id, id, dto);
    }

    @Delete(':id')
    async remove(@Param('id') id: string, @Request() req) {
        assertPlatformAdmin(req.user);
        return this.service.deleteTenant(req.user.id, id);
    }
    ...
    async startIntervention(@Param('id') id: string, @Request() req) {
        assertPlatformAdmin(req.user);
        return this.service.startIntervention(req.user.id, id);
    }
}
...
export class PlatformUsersController {
    ...
    @Post()
    async create(@Body() dto: CreatePlatformAdminDto, @Request() req) {
        assertPlatformAdmin(req.user);
    ...
    async suspend(@Param('id') id: string, @Request() req) {
        assertPlatformAdmin(req.user);
        return this.service.suspendUser(req.user.id, id);
    }

    @Post(':id/reset-password')
    async resetPassword(@Param('id') id: string, @Body('password') password: string, @Request() req) {
        assertPlatformAdmin(req.user);
        return this.service.resetPassword(req.user.id, id, password);
    }
}
...
export class SubscriptionsController {
    ...
    @Post(':id/force-renewal')
    async forceRenewal(@Param('id') id: string, @Request() req) {
        assertPlatformAdmin(req.user);
        return this.service.forceRenewal(req.user.id, id);
    }

    @Patch(':id/override')
    async override(@Param('id') id: string, @Body() dto: BillingOverrideDto, @Request() req) {
        assertPlatformAdmin(req.user);
        return this.service.overrideBillingCycle(req.user.id, id, dto);
    }
}
