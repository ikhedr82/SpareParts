...
import { UpdateTenantAdminDto, CreatePlatformAdminDto, BillingOverrideDto } from './dto/tenant-management.dto';
...
export class TenantAdminService {
    ...
    async startIntervention(adminUserId: string, tenantId: string) {
        // High-security action: start an intervention session
        await this.auditService.logAction(
            'PLATFORM',
            adminUserId,
            'DIRECT_INTERVENTION_START',
            'Tenant',
            tenantId,
            undefined,
            { sessionStarted: true, protocol: 'EMERGENCY_OVERRIDE' }
        );
        return { success: true, message: 'Intervention protocol initiated and logged' };
    }

    async updateTenant(adminUserId: string, tenantId: string, dto: UpdateTenantAdminDto) {
        const tenant = await this.prisma.tenant.findUnique({ where: { id: tenantId } });
        if (!tenant) throw new NotFoundException('Tenant not found');
    ...
    async deleteTenant(adminUserId: string, tenantId: string) {
        const tenant = await this.prisma.tenant.findUnique({ where: { id: tenantId } });
        if (!tenant) throw new NotFoundException('Tenant not found');
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            await this.auditService.logAction(
                ...
                { status: 'TERMINATED', deleted: true },
                undefined,
                undefined,
                tx
            );
    ...
    async createUser(adminUserId: string, dto: CreatePlatformAdminDto) {
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            await this.auditService.logAction(
                ...
                user.id,
                null,
                { email: user.email, isPlatformUser: user.isPlatformUser },
                undefined,
                undefined,
                tx
            );
    ...
    async resetPassword(adminUserId: string, userId: string, newPassword?: string) {
        const user = await this.prisma.user.findUnique({ where: { id: userId } });
        if (!user) throw new NotFoundException('User not found');

        const passwordToSet = newPassword || 'Password123!'; // Emergency reset
    ...
    async getRevenueTrends() {
        ...
        const trends = invoices.reduce((acc: any, inv) => {
            ...
            acc[key] += Number(inv.amount);
            return acc;
        }, {});

        return Object.entries(trends).map(([name, total]) => ({ name, total }));
    }

    async forceRenewal(adminUserId: string, subscriptionId: string) {
        const sub = await this.prisma.subscription.findUnique({ where: { id: subscriptionId } });
        if (!sub) throw new NotFoundException('Subscription not found');

        return this.prisma.$transaction(async (tx) => {
            // Logic to create a manual invoice and extend period
            const newPeriodEnd = new Date(sub.currentPeriodEnd || new Date());
    ...
    async overrideBillingCycle(adminUserId: string, subscriptionId: string, dto: BillingOverrideDto) {
        const sub = await this.prisma.subscription.findUnique({ where: { id: subscriptionId } });
        if (!sub) throw new NotFoundException('Subscription not found');
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            if (dto.nextBillingDate) data.currentPeriodEnd = new Date(dto.nextBillingDate);
