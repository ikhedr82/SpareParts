...
/**
 * Seed Master Data for Automotive Parts
 * All operations are idempotent (safe to re-run)
 */

// ==========================================
// GLOBAL CATALOG DATA (Not tenant-specific)
// ==========================================
...
// Real automotive products with actual part numbers from research
 ...
 */
export async function seedMasterData(prisma: PrismaClient, tenantId: string, branchId: string) {
    console.log('🌱 Seeding Automotive Master Data...');

    // Step 1: Seed Brands (Global)
    console.log('  ✓ Seeding brands...');
    const brandMap: Record<string, any> = {};
    for (const brandData of automotiveBrands) {
    ...
    // Step 2: Seed Product Categories (Global) with hierarchy
    console.log('  ✓ Seeding product categories...');
    const categoryMap: Record<string, any> = {};

    // First pass: Create root categories
    for (const catData of productCategoriesHierarchy.filter(c => !c.parentName)) {
    ...
    // Step 3: Seed Tax Rates (Tenant-specific)
    ...
    // Step 4: Seed Products (Global) with tax rate links
    ...
    for (const productData of automotiveProducts) {
        const product = await prisma.product.upsert({
            ...
            create: {
                ...
                taxRateId: taxRateMap['Standard VAT'].id, // Default to standard VAT
                status: 'ACTIVE',
            },
        });
    ...
    // Step 5: Seed Suppliers (Tenant-specific)
    ...
    // Step 6: Seed Customers (Tenant-specific)
    ...
    // Step 7: Seed Initial Inventory (Branch-specific)
    console.log('  ✓ Seeding initial inventory...');

    // Select 30 popular products to stock
    const productsToStock = [
        // High-demand filters and fluids
    ...
    for (const stockItem of productsToStock) {
        ...
        await prisma.inventory.upsert({
            ...
            create: {
                ...
                barcode: `${product.id.substring(0, 12).replace(/-/g, '')}`, // Simple barcode from ID
            },
        });
    }

    console.log('✅ Automotive Master Data Seeding Complete!');
    console.log(`   - Brands: ${automotiveBrands.length}`);
    console.log(`   - Categories: ${productCategoriesHierarchy.length}`);
    console.log(`   - Products: ${automotiveProducts.length}`);
    console.log(`   - Tax Rates: ${taxRatesData.length}`);
