"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.MahleExtractor = void 0;
const base_extractor_1 = require("../base-extractor");
class MahleExtractor extends base_extractor_1.BaseExtractor {
    constructor(outputDirectory = 'src/catalog/data/raw/mahle') {
        super({
            manufacturerName: 'Mahle',
            sourceType: 'WEB_CATALOG',
            sourceUrl: 'https://www.mahle-aftermarket.com/catalog',
            outputDirectory,
        });
    }
    async extract() {
        this.log('Starting Mahle catalog extraction...');
        const startTime = Date.now();
        const products = [];
        const errors = [];
        try {
            const mahleProducts = this.getCuratedMahleProducts();
            for (const product of mahleProducts) {
                try {
                    products.push(product);
                }
                catch (error) {
                    const errorMsg = `Failed to process product ${product.partNumber}: ${error}`;
                    this.logError(errorMsg, error);
                    errors.push(errorMsg);
                }
            }
            const result = {
                manufacturer: 'Mahle',
                timestamp: new Date(),
                totalProducts: products.length,
                products,
                errors,
            };
            const filepath = await this.saveRawData(result);
            this.log(`Extraction complete! ${products.length} products extracted.`);
            this.log(`Raw data saved to: ${filepath}`);
            const duration = ((Date.now() - startTime) / 1000).toFixed(2);
            this.log(`Extraction took ${duration}s`);
            return result;
        }
        catch (error) {
            this.logError('Fatal extraction error', error);
            throw error;
        }
    }
    getCuratedMahleProducts() {
        return [
            {
                partNumber: 'OC 205',
                name: 'Mahle Original Oil Filter OC 205',
                description: 'Premium spin-on oil filter',
                category: 'Oil Filters',
                brand: 'Mahle',
                weight: 0.35,
                dimensions: '4.7" x 3.5"',
                images: [],
                alternatePartNumbers: ['OC205', 'OX205D'],
                fitment: [
                    { make: 'Toyota', model: 'Camry', yearStart: 2012, yearEnd: 2017, engineType: '2.5L 4-cyl' },
                    { make: 'Toyota', model: 'RAV4', yearStart: 2013, yearEnd: 2018, engineType: '2.5L 4-cyl' },
                    { make: 'Lexus', model: 'ES250', yearStart: 2013, yearEnd: 2018, engineType: '2.5L 4-cyl' },
                ],
            },
            {
                partNumber: 'OC 264',
                name: 'Mahle Oil Filter OC 264',
                description: 'High-efficiency oil filtration',
                category: 'Oil Filters',
                brand: 'Mahle',
                weight: 0.32,
                dimensions: '4.5" x 3.3"',
                images: [],
                alternatePartNumbers: ['OC264', 'OX264D'],
                fitment: [
                    { make: 'Honda', model: 'Civic', yearStart: 2012, yearEnd: 2015, engineType: '1.8L 4-cyl' },
                    { make: 'Honda', model: 'Accord', yearStart: 2013, yearEnd: 2017, engineType: '2.4L 4-cyl' },
                ],
            },
            {
                partNumber: 'OC 90',
                name: 'Mahle Oil Filter OC 90',
                description: 'OE quality oil filter for European vehicles',
                category: 'Oil Filters',
                brand: 'Mahle',
                weight: 0.4,
                dimensions: '5.2" x 3.8"',
                images: [],
                alternatePartNumbers: ['OC90', 'OX90D'],
                fitment: [
                    { make: 'BMW', model: '3 Series', yearStart: 2012, yearEnd: 2018, engineType: '2.0L 4-cyl Turbo' },
                    { make: 'BMW', model: '4 Series', yearStart: 2014, yearEnd: 2020, engineType: '2.0L 4-cyl Turbo' },
                ],
            },
            {
                partNumber: 'LX 2633',
                name: 'Mahle Air Filter LX 2633',
                description: 'Premium engine air filter',
                category: 'Air Filters',
                brand: 'Mahle',
                weight: 0.38,
                dimensions: '11.8" x 8.7" x 1.6"',
                images: [],
                alternatePartNumbers: ['LX2633'],
                fitment: [
                    { make: 'Toyota', model: 'Camry', yearStart: 2012, yearEnd: 2017, engineType: '2.5L 4-cyl' },
                    { make: 'Toyota', model: 'RAV4', yearStart: 2013, yearEnd: 2018, engineType: '2.5L 4-cyl' },
                ],
            },
            {
                partNumber: 'LX 1780',
                name: 'Mahle Air Filter LX 1780',
                description: 'High-flow air filtration',
                category: 'Air Filters',
                brand: 'Mahle',
                weight: 0.35,
                dimensions: '10.5" x 8" x 1.5"',
                images: [],
                alternatePartNumbers: ['LX1780'],
                fitment: [
                    { make: 'Honda', model: 'Civic', yearStart: 2012, yearEnd: 2015, engineType: '1.8L 4-cyl' },
                    { make: 'Honda', model: 'Accord', yearStart: 2013, yearEnd: 2017, engineType: '2.4L 4-cyl' },
                ],
            },
            {
                partNumber: 'KL 756',
                name: 'Mahle Fuel Filter KL 756',
                description: 'In-line fuel filter',
                category: 'Fuel Filters',
                brand: 'Mahle',
                weight: 0.15,
                dimensions: '3.5" x 2.2"',
                images: [],
                alternatePartNumbers: ['KL756', 'KLH756'],
                fitment: [
                    { make: 'BMW', model: '3 Series', yearStart: 2007, yearEnd: 2013, engineType: '3.0L 6-cyl' },
                    { make: 'BMW', model: '5 Series', yearStart: 2008, yearEnd: 2010, engineType: '3.0L 6-cyl' },
                ],
            },
            {
                partNumber: 'KL 440',
                name: 'Mahle Fuel Filter KL 440',
                description: 'Diesel fuel filter',
                category: 'Fuel Filters',
                brand: 'Mahle',
                weight: 0.25,
                dimensions: '4.2" x 3"',
                images: [],
                alternatePartNumbers: ['KL440', 'KLH440'],
                fitment: [
                    { make: 'Mercedes-Benz', model: 'Sprinter', yearStart: 2010, yearEnd: 2018, engineType: '2.1L 4-cyl Diesel' },
                ],
            },
            {
                partNumber: 'LA 230',
                name: 'Mahle Cabin Air Filter LA 230',
                description: 'Activated carbon cabin filter',
                category: 'Air Filters',
                brand: 'Mahle',
                weight: 0.28,
                dimensions: '9.8" x 7.8" x 1.2"',
                images: [],
                alternatePartNumbers: ['LA230', 'LAK230'],
                fitment: [
                    { make: 'Toyota', model: 'Camry', yearStart: 2012, yearEnd: 2017 },
                    { make: 'Toyota', model: 'Avalon', yearStart: 2013, yearEnd: 2018 },
                ],
            },
            {
                partNumber: 'LA 79',
                name: 'Mahle Cabin Air Filter LA 79',
                description: 'Premium cabin air filtration',
                category: 'Air Filters',
                brand: 'Mahle',
                weight: 0.25,
                dimensions: '8.5" x 7" x 1"',
                images: [],
                alternatePartNumbers: ['LA79', 'LAK79'],
                fitment: [
                    { make: 'Honda', model: 'Civic', yearStart: 2012, yearEnd: 2015 },
                    { make: 'Honda', model: 'Accord', yearStart: 2013, yearEnd: 2017 },
                ],
            },
            {
                partNumber: 'LA 368',
                name: 'Mahle Cabin Filter LA 368',
                description: 'Multi-layer cabin air filter',
                category: 'Air Filters',
                brand: 'Mahle',
                weight: 0.3,
                dimensions: '11" x 8.5" x 1.5"',
                images: [],
                alternatePartNumbers: ['LA368'],
                fitment: [
                    { make: 'BMW', model: '3 Series', yearStart: 2012, yearEnd: 2018 },
                    { make: 'BMW', model: '4 Series', yearStart: 2014, yearEnd: 2020 },
                ],
            },
        ];
    }
}
exports.MahleExtractor = MahleExtractor;
//# sourceMappingURL=mahle-extractor.js.map