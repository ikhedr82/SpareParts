"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.RefundsController = void 0;
const common_1 = require("@nestjs/common");
const refunds_service_1 = require("./refunds.service");
const exceptions_dto_1 = require("./dto/exceptions.dto");
let RefundsController = class RefundsController {
    constructor(service) {
        this.service = service;
    }
    async createRefund(req, dto) {
        const tenantId = req.user.tenantId;
        return this.service.createRefund(tenantId, dto.orderId, dto.amount, dto.reason, req.user.id, dto.returnId);
    }
    async processRefund(req, id) {
        const tenantId = req.user.tenantId;
        return this.service.processRefund(tenantId, id, req.user.id);
    }
    async findAll(req, status, orderId) {
        const tenantId = req.user.tenantId;
        const refundStatus = status;
        return this.service.findAll(tenantId, refundStatus, orderId);
    }
};
exports.RefundsController = RefundsController;
__decorate([
    (0, common_1.Post)(),
    __param(0, (0, common_1.Req)()),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, exceptions_dto_1.CreateRefundDto]),
    __metadata("design:returntype", Promise)
], RefundsController.prototype, "createRefund", null);
__decorate([
    (0, common_1.Patch)(':id/process'),
    __param(0, (0, common_1.Req)()),
    __param(1, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String]),
    __metadata("design:returntype", Promise)
], RefundsController.prototype, "processRefund", null);
__decorate([
    (0, common_1.Get)(),
    __param(0, (0, common_1.Req)()),
    __param(1, (0, common_1.Query)('status')),
    __param(2, (0, common_1.Query)('orderId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String, String]),
    __metadata("design:returntype", Promise)
], RefundsController.prototype, "findAll", null);
exports.RefundsController = RefundsController = __decorate([
    (0, common_1.Controller)('logistics/refunds'),
    __metadata("design:paramtypes", [refunds_service_1.RefundsService])
], RefundsController);
//# sourceMappingURL=refunds.controller.js.map